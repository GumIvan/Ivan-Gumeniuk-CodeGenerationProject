...
import warnings
from sklearn.metrics import silhouette_score #Added this library to perform silhouette score calculations
...
#added following code to validify cluster using silhouette scores
sil_scores = []
for k in range(2, 11):
    km_sil = KMeans(n_clusters=k, random_state=42, n_init=10)
    labels = km_sil.fit_predict(X_2d_scaled)
    sil_scores.append(silhouette_score(X_2d_scaled, labels))

# print the score for the chosen k
print(f"Silhouette Score (k={K_OPTIMAL}): {sil_scores[K_OPTIMAL-2]:.4f}")
...
#Changed the names from innitial code to be more meaningful
CLUSTER_NAMES = {
    0: 'Average customers',
...
style_ax(ax_sil, 'Silhouette Score vs k', 'Number of Clusters (k)', 'Silhouette Score')
ax_sil.plot(range(2, 11), sil_scores, 'o-', color='#2ECC71', linewidth=2,
            markersize=6, markerfacecolor='white')
ax_sil.axvline(K_OPTIMAL, color='#E74C3C', linestyle='--',
               linewidth=1.5, label=f'k = {K_OPTIMAL}')
ax_sil.legend(fontsize=8, labelcolor='white', facecolor='#1A1D27', edgecolor='#333344')
...
ax_summary.set_xticklabels([CLUSTER_NAMES[i] for i in range(K_OPTIMAL)], color='#AAAAAA', fontsize=6, rotation=15, ha='right')
...
plt.savefig('kmeans_clusters_v4.png', dpi=150, #Just changed the name of the saved png to clarify differences between version outputs
